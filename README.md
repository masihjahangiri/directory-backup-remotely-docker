# A docker image to automate backup from a directory in remote server via SSH tunnel
You can easily setup a daily backup system with a docker image.\
This boilerplate help you connect to source server via SSH tunnel (by id_rsa) and make a backup every day at 00:00 (the period is customizable by modifying the `config/cronjobs` file).\
